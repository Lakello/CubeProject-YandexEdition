...
namespace CubeProject.UI
{
	public class WordWobble : MonoBehaviour
	{
		...
		[SerializeField] private float _heightOffsetSpeed = 3.3f;
		[SerializeField] private float _widthOffsetSpeed = 2.5f;
		...
		{
			return new Vector2(Mathf.Sin(time * _heightOffsetSpeed), Mathf.Cos(time * _widthOffsetSpeed));
		}
	}
}